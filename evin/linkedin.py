...
class LinkedinOAuth2(BaseOAuth2):
    ...
    def user_details_url(self):
        fields_selectors = list(
            set(["name", "picture", "email", "given_name", "family_name"] + self.setting("FIELD_SELECTORS", []))
        )
    ...
    def get_user_details(self, response):
        ...
        fullname, first_name, last_name = self.get_user_names(
            first_name=get_localized_name(response["given_name"]),
            last_name=get_localized_name(response["family_name"]),
        )
